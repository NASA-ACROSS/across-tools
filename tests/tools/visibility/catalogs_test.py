...
class TestFallbackBrightStars:
    ...
    def test_get_fallback_bright_stars_returns_tuples(
        self, fallback_bright_stars: list[tuple[SkyCoord, float]]
    ) -> None:
        """Test that fallback function returns list of tuples."""
        assert len(fallback_bright_stars) > 0
        for item in fallback_bright_stars:
    ...
    def test_get_fallback_bright_stars_all_bright(
        ...
        for _, mag in fallback_bright_stars:
...
class TestGetBrightStars:
    ...
    def test_get_bright_stars_returns_tuples(self, mock_vizier_patch: MagicMock) -> None:
    ...
    def test_get_bright_stars_disk_cache_integration(
        self, mock_cache_dir_patch: Path, mock_vizier_patch: MagicMock
    ) -> None:
    ...
    def test_get_bright_stars_default_parameters(self, mock_vizier_patch: MagicMock) -> None:
...
class TestGetBrightStarsEdgeCases:
    ...
    def test_get_bright_stars_very_low_magnitude_limit(self, mock_vizier_patch: MagicMock) -> None:
        """Test with very low magnitude limit (only brightest stars)."""
        stars = get_bright_stars(magnitude_limit=0.0)

        # Should have at least some stars (mock has 1)
        assert len(stars) >= 1
